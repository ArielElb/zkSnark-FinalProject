...
mod tests {
    ...
    #[test]
    fn test_modpow_circuit_incorrect() {
        let mut rng = thread_rng();
        let base = random_fe(&mut rng);
        let exponent = random_fe(&mut rng);
        let divisor = random_fe(&mut rng);
        let incorrect_result = random_fe(&mut rng); // Intentionally incorrect
    ...
    fn test_edge_cases() {
        ...
        let zero = Fr::zero();
        let one = Fr::one();
        let circuit_zero_base = modpow_ver_circuit {
            base: zero,
            exponent: one,
            result: zero,
            divisor: one,
        ...
        assert!(circuit_zero_base.generate_constraints(cs.clone()).is_ok());
        assert!(cs.is_satisfied().unwrap());
    }
}
